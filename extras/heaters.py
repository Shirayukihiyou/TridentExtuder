...
class Heater:
    def __init__(self, config, sensor):
        ...
        algos = {'watermark': ControlPIDcooler, 'pid': ControlPID}
...
######################################################################
#COOLER                Proportional Integral Derivative (PID) control algo
######################################################################

PID_SETTLE_DELTA = 1.
PID_SETTLE_SLOPE = .1

class ControlPIDcooler:
    def __init__(self, heater, config):
        self.heater = heater
        self.heater_max_power = heater.get_max_power()
        self.Kp = config.getfloat('pid_Kp') / PID_PARAM_BASE
        ...
        self.prev_temp_deriv = 0.
        self.prev_temp_integ = 0.
    def temperature_update(self, read_time, tempreal, target_tempreal):
        temp=-tempreal
        target_temp=-target_tempreal
        time_diff = read_time - self.prev_temp_time
        # Calculate change of temperature
        temp_diff = temp - self.prev_temp
        if time_diff >= self.min_deriv_time:
